...
doc ///
  Key
    fiberGraphGenerator
  Headline
    this method constructs the relevant fibers of a configuration matrix as graphs
  Usage
    G = fiberGraphGenerator(A)
  Inputs
    A : Matrix
  ...
  Description
    Text
      this method constructs the relevant fibers of a configuration matrix A using a recursive algorithm - the fibres are formatted as a list of graphs where two vectors in a fiber share an edge if their supports have non-trivial intersection
    Example
