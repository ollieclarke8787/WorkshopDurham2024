...
fiberGraph Matrix := opts -> A -> (
		if not A.cache#?"FiberGraph" then (
				if opts.CheckInput then (
						...
						-- Def: \NN A pointed if 0 is the only invertible element of \NN A
						--
						-- ker is generated by g1 = (1,1,-1), g2 = (1,-1,1) => (2,0,0) in ker
						-- but if we specify that gens are in Hermite normal form then
						-- is pointed equivalent to generators (HNF) lie in \NN^n
						--
						-- Qu does M2 always give generators of ZZ modules in HNF [probably yes]
